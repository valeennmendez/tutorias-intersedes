...
export default function Home() {
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				const [tutoriasRes] = await Promise.all([axiosInstance.get("/tutorias")]);
		...
		<div>
			<div className="container mx-auto space-y-6 p-6">
				...
				<div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
					<div>
						<h1 className="text-2xl font-bold text-foreground">¡Hola, {user?.nombre}!</h1>
						<p className="text-muted-foreground">Bienvenido al Sistema de Tutorías UNNOBA</p>
					</div>
				...
				<div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
					<Card>
						<CardHeader className="flex flex-row items-center justify-between pb-2">
							<CardTitle className="text-sm font-medium text-muted-foreground">Tutorías Disponibles</CardTitle>
					...
					<Card>
						<CardHeader className="flex flex-row items-center justify-between pb-2">
							<CardTitle className="text-sm font-medium text-muted-foreground">Mi Rol</CardTitle>
					...
					<Card>
						<CardHeader className="flex flex-row items-center justify-between pb-2">
							<CardTitle className="text-sm font-medium text-muted-foreground">Postulaciones</CardTitle>
										...
										<div key={tutoria.id} className="rounded-lg border p-4 transition-colors hover:bg-muted/50">
											<div className="flex items-start justify-between gap-3">
												<div className="space-y-1">
													...
													<p className="text-sm text-muted-foreground">{tutoria.materia?.nombre || "—"}</p>
													{tutoria.fecha && (
											...
											{expandedTutoriaId === tutoria.id && (
												<div className="mt-3 space-y-2 rounded-md border border-slate-200 bg-slate-50 p-3 text-sm text-slate-600">
													<p>
														{tutoria.descripcion ||
															"Podés ver información adicional de esta tutoría al abrir la sección de tutorías completas."}
													</p>
						...
						<CardHeader>
							<div className="flex items-center justify-between">
								...
								<Button variant="outline" size="sm" asChild>
									<Link
										to={
											user?.role === "tutor" || user?.role === "admin" ? "/dashboard/gestionar-avisos" : "/dashboard/bandeja-avisos"
										}
									>
										Ver todos
									</Link>
