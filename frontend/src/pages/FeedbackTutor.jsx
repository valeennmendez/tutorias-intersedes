...
const FEEDBACK_INICIAL = [
  {
    ...
    calificacion: 5,
    comentario:
      "Explica con mucha claridad y usa ejemplos concretos. Hace que la materia se entienda mejor.",
    fecha: "2026-06-15",
    esAnonimo: true,
  },
  {
    ...
    calificacion: 4,
    comentario:
      "Muy ordenado para dar la clase y siempre conecta teoría con ejercicios prácticos.",
    fecha: "2026-06-12",
    esAnonimo: true,
  },
  {
    ...
    calificacion: 4,
    comentario:
      "Acompaña bien los ejercicios y corrige los errores con paciencia. Muy útil para practicar.",
    fecha: "2026-06-14",
    esAnonimo: true,
  },
];
...
export default function FeedbackTutorPage() {
  ...
  const profesorActivo = useMemo(
    () =>
      PROFESORES_DISPONIBLES.find(
        (profesor) => profesor.id.toString() === profesorSeleccionado,
  ...
  const promedioProfesor = useMemo(() => {
    ...
    const suma = feedbackDelProfesor.reduce(
  ...
  const handleProfesorChange = (event) => {
    ...
    const profesorNuevo = PROFESORES_DISPONIBLES.find(
      (profesor) => profesor.id.toString() === nuevoProfesorId,
    );
        ...
        <div className="mx-auto flex w-full max-w-5xl flex-col gap-6">
          <div className="flex flex-col gap-2 text-center sm:text-left">
            ...
            <p className="text-sm text-slate-600">
              Elegí un profesor en particular, revisá sus materias y leé solo sus reseñas.
              Las opiniones se publican de forma anónima.
            </p>
          </div>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <Card className="border-slate-200 shadow-md lg:order-1">
              <CardHeader>
                ...
                <CardDescription>
                  Contá cómo enseña este profesor y cómo fue su clase.
                </CardDescription>
              </CardHeader>
              ...
              <CardContent className="space-y-4 pt-0">
                <form onSubmit={handleEnviarFeedback} className="space-y-4">
                  ...
                  <Button
                    type="submit"
                    disabled={enviando}
                    className="w-full bg-[#008BBA] text-white hover:bg-[#00779f]"
                  >
            ...
            <div className="space-y-4 lg:order-2">
              <div className="rounded-xl bg-sky-50 px-4 py-3 text-sm text-sky-900">
                ...
                <p className="mt-1 text-xs">
                  No se muestra tu nombre ni tu correo. Solo aparece la opinión sobre el profesor
                  y la materia.
                </p>
              </div>
              ...
              <Card className="border-slate-200 shadow-md">
                <CardHeader>
                  <CardTitle className="text-lg text-slate-900">
                    Reseñas de {profesorActivo.nombre}
                  </CardTitle>
                ...
                <CardContent className="space-y-4 pt-0">
                  ...
                  {feedbackDelProfesor.length === 0 ? (
                    <p className="text-sm text-slate-600">
                      Aún no hay reseñas para este profesor. ¡Sé el primero!
                    </p>
                  ) : (
                    feedbackDelProfesor.map((item) => (
                      <div
                        key={item.id}
                        className="space-y-2 border-b border-slate-200 pb-4 last:border-0"
                      >
