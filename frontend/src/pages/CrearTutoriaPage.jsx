...
function CrearTutoriaPage() {
	...
	const { id: tutoriaId } = useParams();
	const editando = Boolean(tutoriaId);
	...
	useEffect(() => {
		const fetchData = async () => {
			...
			try {
				const response = await axiosInstance.get(`/tutorias/${tutoriaId}`);
				const data = response.data;

				setMateriaSeleccionada(data.materiaId?.toString() || "");
				setSedeSeleccionada(data.sede || "");
				setSeleccion(data.modalidad === "PRESENCIAL" ? "P" : data.modalidad === "VIRTUAL" ? "V" : null);
				setUbiOrLink(data.ubicacion || data.linkVirtual || "");
				setDataForm({
					nombre: data.nombre || "",
				...
				toast.error("No se pudo cargar la tutoría para editar");
			}
		};
	...
	const handleSubmit = async (event) => {
		...
		const payload = {
			...
			cupo: dataForm.cupo === "" || dataForm.cupo == null ? null : Number(dataForm.cupo),
		};

		if (editando) {
			await tutoriaStore.getState().actualizarTutoria(tutoriaId, payload);
			navigate("/dashboard/panel-tutor");
			return;
		}

		const creada = await tutoriaStore.getState().crearTutoria(payload);
		if (creada) {
			navigate("/dashboard/panel-tutor");
		}
	};
			...
			<div className="mx-auto flex w-full max-w-5xl flex-col gap-6">
				<div className="flex items-center justify-between gap-4">
					<div>
						...
						<h1 className="text-2xl font-bold text-slate-900">{editando ? "Editar Tutoría" : "Nueva Tutoría"}</h1>
						<p className="text-sm text-slate-500">
							{editando ? "Actualiza los datos de la tutoría" : "Crear nueva sesión de tutoría para tus compañeros"}
						</p>
					</div>
				</div>
					...
					<div className="px-6 py-6">
						<form onSubmit={handleSubmit} className="space-y-5">
							...
							<div className="flex gap-3 pt-2">
								<button type="submit" className="cursor-pointer rounded-lg bg-[#008BBA] px-5 py-2.5 font-medium text-white hover:bg-[#00779f]">
									{editando ? "Guardar cambios" : "Crear Tutoría"}
								</button>
